...
<div class="flex flex-col justify-between bg-grey-2 w-72 h-full">
    <div class="flex flex-col space-y-8 px-4 py-6">
        ...
        <div class="flex flex-col space-y-4 text-white text-xl">
            ...
            <a class="<?= $active == 'theatres'? 'bg-white text-blue-4' : '' ?> flex flex-row items-center py-3 px-4 rounded-xl space-x-3 hover:bg-white hover:text-blue-4" href="<?= HOST ?>theatres">
                <i class="fas fa-theater-masks"></i>
                <span class="link">Théatres</span>
            </a>
            <a class="<?= $active == 'concerts'? 'bg-white text-blue-4' : '' ?> flex flex-row items-center py-3 px-4 rounded-xl space-x-3 hover:bg-white hover:text-blue-4" href="<?= HOST ?>concerts">
                <i class="fas fa-guitar"></i>
                <span class="link">Concerts</span>
            </a>
            <a class="<?= $active == 'humours'? 'bg-white text-blue-4' : '' ?> flex flex-row items-center py-3 px-4 rounded-xl space-x-3 hover:bg-white hover:text-blue-4" href="<?= HOST ?>humours">
                <i class="fas fa-laugh-beam"></i>
                <span class="link">Humours</span>
            </a>
            <a class="<?= $active == 'expositions'? 'bg-white text-blue-4' : '' ?> flex flex-row items-center py-3 px-4 rounded-xl space-x-3 hover:bg-white hover:text-blue-4" href="<?= HOST ?>expositions">
                <i class="fas fa-palette"></i>
                <span class="link">Expositions</span>
            </a>
            <?php if (isset($_SESSION['user']) && !empty($_SESSION['user'])) { ?>
                <a class="<?= $active == 'reservations'? 'bg-white text-blue-4' : '' ?> flex flex-row items-center py-3 px-4 rounded-xl space-x-3 hover:bg-white hover:text-blue-4" href="<?= HOST ?>reservations">
                    <i class="fas fa-ticket-alt"></i>
                    <span class="link">Mes Réservations</span>
                </a>
            <?php } ?>
