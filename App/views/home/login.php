...
<section class="flex flex-col px-20 space-y-8 items-center justify-center h-full w-full">
    ...
    <div class="flex flex-col bg-grey-3 rounded-xl h-2/5 w-1/3 text-white px-4 py-4 space-y-6">
        <div class="flex flex-col items-center space-y-2">
            <span class="text-3xl">Connexion</span>
            <span class="text-xl">Pas encore de compte ? <a class="text-blue-4" href="<?= HOST ?>register">S'enregistrer</a></span>
        </div>
        ...
        <form class="flex flex-col flex-1 items-center space-y-4 w-full h-full justify-evenly" action="<?= HOST ?>login" method="post">

            <div class="flex flex-row items-center space-x-3 h-10 px-4 w-3/4 bg-grey-2 rounded-lg">
                <i class="fas fa-at "></i>
                <input class="outline-none h-full flex-1 bg-grey-2 pl-1" type="email" name="mail" id="" placeholder="E-Mail" <?php if (!empty($_SESSION['mail'])) print_r("value=\"".$_SESSION['mail']."\""); ?> required>
            </div>
            ...
            <input class="rounded-lg bg-blue-4 py-2 px-4 text-white cursor-pointer hover:bg-white hover:text-blue-4" type="submit" value="Se connecter">
        </form>
    </div>
</section>
